...
export interface IResolverProvider {
    ...
    /**
     * The connection library containing the specific connections to the networks. If none is provided the default connection library is used.
     */
    ...
    /**
     * Given a wallet address returns the NFT tokenId uint256 string.
    ...
    /**
     * Calls the blockchain registry to approve the `addressToApprove` to the `resource` NFT eg. to give the address permission to transfer. Usually the `signer` must be the owner of the resolved resource NFT.
     * @param resource an `IResolvedResource` instance.
     * @param addressToApprove the address to approve.
     * @param signer a `ether.Signer` wallet. If `signer.provider` is `undefined` the `provider` of the resolved resource's `resolverProvider` is used.
     */
    setApproved(resource: IResolvedResource, addressToApprove: string, signer: ethers.Signer): Promise<boolean>;
     ...
     * @param tokenId the NFT tokenId uint256 string.
     * @param network the network where the NFT is located. If not available every network is scanned to search the `tokenId` NFT.
